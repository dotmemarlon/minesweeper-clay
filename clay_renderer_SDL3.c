 ...
 * user data parameter */
static TTF_Font *g_fonts[1];
/* Global for convenience. Even in 4K this is enough for smooth curves (low radius or rect size coupled with
 * no AA or low resolution might make it appear as jagged curves) */
static int NUM_CIRCLE_SEGMENTS = 16;

typedef enum {
	MYCLAY_RENDER_COMMAND_TYPE_BORDER_2_COLOR
} MyClay_CustomElementType;

typedef struct {
	Clay_Color color1;
	Clay_Color color2;
	int size;
} MyClay_Border2ColorElementConfig;

typedef struct {
	void* data;
	MyClay_CustomElementType element_type;
} MyClay_CustomElementConfig;
    ...
    for (size_t i = 0; i < rcommands->length; i++) {
        ...
        switch (rcmd->commandType) {
						case CLAY_RENDER_COMMAND_TYPE_CUSTOM: {
							MyClay_CustomElementConfig *custom_config = rcmd->renderData.custom.customData;
							switch (custom_config->element_type) {

								case MYCLAY_RENDER_COMMAND_TYPE_BORDER_2_COLOR: {
									MyClay_Border2ColorElementConfig * config = custom_config->data;
									SDL_SetRenderDrawColor(renderer, config->color1.r, config->color1.g, config->color1.b, config->color1.a);
									for (int i = 0;i < config->size; ++i) {
										SDL_RenderLine(renderer, rect.x + i, rect.y, rect.x + i, rect.y + rect.h-i-1);
										SDL_RenderLine(renderer, rect.x, rect.y + i, rect.x + rect.w - i - 1, rect.y + i);
									}
									SDL_SetRenderDrawColor(renderer, config->color2.r, config->color2.g, config->color2.b, config->color2.a);
									for (int i = 0;i < config->size; ++i) {
										SDL_RenderLine(renderer, rect.x + rect.w - i, rect.y + i + 1, rect.x + rect.w - i, rect.y + rect.h);
										SDL_RenderLine(renderer, rect.x + i + 1, rect.y + rect.h - i, rect.x + rect.w, rect.y + rect.h - i);
									}
								} break;
							}
						} break;
						case CLAY_RENDER_COMMAND_TYPE_IMAGE: {
							SDL_Texture* image_texture = rcmd->renderData.image.imageData;
							//Clay_Color tint_color = rcmd->renderData.image.backgroundColor;
							SDL_FRect srcrect = {0,0,rcmd->renderData.image.sourceDimensions.width,rcmd->renderData.image.sourceDimensions.height};
